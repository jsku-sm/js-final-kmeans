...
tab1, tab2, tab3, tab4 = st.tabs(["표준화 평균 프로파일", "원점수 평균", "PCA 산점도", "레이더 차트"])
...
with tab1:
    ...
    fig_heat = px.imshow(
        ...
        text_auto=".2f",
        aspect="auto",
        title="군집별 표준화 평균 프로파일",
        labels=dict(x="요인", y="군집", color="표준화 점수"),
    )
    fig_heat.update_layout(height=450)
    st.plotly_chart(fig_heat, use_container_width=True)
    st.caption("0보다 크면 전체 평균보다 높은 편, 0보다 작으면 전체 평균보다 낮은 편입니다.")

with tab2:
    raw_profile_display = rename_for_display(profile_raw.copy()).round(2)
    st.dataframe(raw_profile_display, use_container_width=True)
    raw_long = raw_profile_display.reset_index().melt(id_vars="cluster", var_name="요인", value_name="평균")
    fig_raw = px.bar(raw_long, x="cluster", y="평균", color="요인", barmode="group", title="군집별 원점수 평균")
    fig_raw.update_layout(xaxis_title="군집", yaxis_title="5점 척도 평균", height=450)
...
with tab3:
    if len(selected_vars) >= 2:
        ...
        fig_pca = px.scatter(
            ...
            title=f"PCA 2차원 군집 산점도  |  설명분산: PC1 {pca.explained_variance_ratio_[0]*100:.1f}%, PC2 {pca.explained_variance_ratio_[1]*100:.1f}%",
        )
